...
const resolveSuccess = (payload: UpstreamServicesResponse) => {
...
export async function GET(request: NextRequest) {
    try {
        ...
        )) as UpstreamServicesResponse

        if (!resolveSuccess(payload)) {
            return NextResponse.json(
                {
                    status: false,
                    message: payload.message || 'Failed to fetch services',
