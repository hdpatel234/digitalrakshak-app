...
const resolveSuccess = (payload: UpstreamSearchResponse) => {
...
export async function GET(request: NextRequest) {
    try {
        ...
        )) as UpstreamSearchResponse

        if (!resolveSuccess(payload)) {
            return NextResponse.json(
                {
                    status: false,
                    message: payload.message || 'Failed to fetch search data',
        ...
        return NextResponse.json(
            {
                status: true,
                message: payload.message || 'Search data fetched successfully',
                ...(payload.data ? { data: payload.data } : {}),
            },
            { status: 200 },
        )
