...
export async function GET(request: NextRequest) {
    try {
        ...
        const payload = (await apiClient.request('get', 'client/tickets', null, false, {
            headers: buildAuthHeaders(tokenType, accessToken),
            params,
        })) as UpstreamResponse
        ...
        if (!resolveSuccess(payload)) {
            return NextResponse.json(
                {
                    status: false,
                    message: payload.message || 'Failed to fetch tickets',
                    ...(payload.data ? { data: payload.data } : {}),
                },
                { status: payload.status_code || 400 },
            )
        ...
        return NextResponse.json(
            {
                status: true,
                message: payload.message || 'Tickets fetched successfully',
                ...(payload.data ? { data: payload.data } : {}),
            },
            { status: 200 },
        )
