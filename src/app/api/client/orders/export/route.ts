...
export async function GET(request: NextRequest) {
    try {
        ...
        // Pass all search parameters to upstream
        const searchParams = new URLSearchParams(request.nextUrl.searchParams.toString())

        const upstreamUrl = `${resolveApiBaseUrl()}/client/orders/export?${searchParams.toString()}`
        ...
        if (!upstreamResponse.ok) {
            ...
            try {
                ...
                console.log("UPSTREAM ERROR STATUS:", upstreamResponse.status)
                console.log("UPSTREAM ERROR BODY:", fallbackText)
                ...
                console.error("Error reading upstream response:", e)
            }
        ...
        const defaultFilename = `orders-export.${format}`
        ...
        return NextResponse.json(
            {
                status: false,
                message: responseError?.message || 'Failed to download export file',
            },
            { status: 500 },
        )
    }
