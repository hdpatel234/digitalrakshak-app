...
export async function GET(request: NextRequest) {
    try {
        ...
        const payload = (await apiClient.request('get', 'client/settings/company', null, false, {
        ...
        if (!resolveSuccess(payload)) {
            return NextResponse.json(
                {
                    status: false,
                    message: payload.message || 'Failed to fetch company details',
        ...
        return NextResponse.json(
            {
                status: true,
                message: payload.message || 'Company details fetched successfully',
                ...(payload.data ? { data: payload.data } : {}),
            },
            { status: 200 },
        )
...
export async function PUT(request: NextRequest) {
    try {
        ...
        const payload = (await apiClient.request('put', 'client/settings/company', body, false, {
            headers: buildAuthHeaders(tokenType, accessToken),
        })) as UpstreamResponse
        ...
        if (!resolveSuccess(payload)) {
            return NextResponse.json(
                {
                    status: false,
                    message: payload.message || 'Failed to update company details',
                    ...(payload.data ? { data: payload.data } : {}),
                },
                { status: payload.status_code || 400 },
            )
        ...
        return NextResponse.json(
            {
                status: true,
                message: payload.message || 'Company details updated successfully',
                ...(payload.data ? { data: payload.data } : {}),
            },
            { status: 200 },
        )
