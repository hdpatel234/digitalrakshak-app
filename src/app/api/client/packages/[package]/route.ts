...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ package: string }> }) {
    try {
        const { package: packageId } = await params
        const session = await auth()
        const accessToken = session?.accessToken || ''
        ...
        if (!resolveSuccess(payload)) {
            return NextResponse.json(
                {
                    status: false,
                    message: payload.message || 'Failed to delete package',
                    ...(payload.data ? { data: payload.data } : {}),
                    ...(payload.error ? { error: payload.error } : {}),
                },
                { status: payload.status_code || 400 },
        ...
        return NextResponse.json(
            {
                status: true,
                message: payload.message || 'Package deleted successfully',
                ...(payload.data ? { data: payload.data } : {}),
            },
            { status: 200 },
        )
